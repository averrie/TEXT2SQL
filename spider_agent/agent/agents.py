...
class PromptAgent:
    ...
    def self_consistency_predict(self, obs: Dict = None, num_samples=5) -> List:
        ...
        temperature_values = [0.3, 0.5, 0.6, 0.7, 0.9]
        ...
        for i in range(num_samples):
            status = False
            local_temp = temperature_values[i % len(temperature_values)]
            local_top_p = top_p_values[i % len(top_p_values)]
        ...
        analysis_prompt = f"""
        You are a self-consistency analysis assistant.
        The observation is: {str(obs)}
        
        We have {num_samples} responses:
        {responses}
        ...
        Please choose the best final response
        and reprint it exactly in the same 'Thought:...' + 'Action:...' format.
        
        Best response:
        """
        final_messages = [{"role": "user", "content": analysis_prompt}]
        ...
        while not done and step_idx < self.max_steps:
            ...
            if action is None:
                ...
                if last_action is not None and last_action == action:
                    ...
                    if "Action execution time exceeded!" in obs:
                        logger.error("Timeout reached, terminating run loop.")
                        done = True
                        result = obs
